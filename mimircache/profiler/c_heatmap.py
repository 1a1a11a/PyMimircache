...
def server_plot_all(path="../data/cloudphysics/", num_of_threads=48):
    ...
    for filename in os.listdir(path):
        ...
        if filename.endswith('.vscsitrace'):
            ...
            # if os.path.exists("0622/" + filename + "LRU_Optimal_" + str(mem_size) + "_r.png"):
            if os.path.exists("0622/" + '_'.join([filename, "_LRU_Optimal_", str(int(1000 * (1.6 ** 15))), MODE]) + "_.png"):
                continue

            # t1 = time.time()
            # hm.heatmap(reader, MODE, TIME_INTERVAL, "rd_distribution", num_of_threads=num_of_threads, figname=
            # "0620/" + '_'.join([filename, "_rd_distribution_", MODE]) + "_.png")
...
def localtest():
    ...
    print('a')
    hm.heatmap(reader, MODE, TIME_INTERVAL, "rd_distribution", num_of_threads=NUM_OF_THREADS, figname="rd_dist_.png")

    print('b')
    hm.differential_heatmap(reader, MODE, TIME_INTERVAL, PLOT_TYPE, "LRU", cache_size=CACHE_SIZE,
