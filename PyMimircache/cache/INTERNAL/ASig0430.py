...
class ASig0430(Cache):
    def __init__(self, cache_size, **kwargs):
        ...
        self.output_log = open("ASig0430", "w")
    ...
    def evict(self, **kwargs):
        ...
        if self.n_evictions % (self.cache_size) == 1:
            for i in range(self.freq_boundary[0]):
                ...
                #     print("ASig0430 ts {} lowFreq{} not ready, {}".format(self.ts, i, ", ".join([str(len(i)) for i in self.low_freq_ts_count])))
                #     continue
                #
                ...
                # except Exception as e:
                #     self.failed_fit_count += 1
                #     print("ASig0430 ts {} lowFreq{} fitting failed".format(self.ts, i))
                #     pass
        ...
        if not CHANGE_HF:
            if self.ts and self.ts % (self.cache_size * 20) == 0:
                ...
                for k in to_remove:
                    del self.hf_dict[k]
                # if to_remove:
                #     print("ASig0430 {} remove {} left {}".format(self.ts, to_remove, self.hf_dict.items()))


        if self.ts and self.ts % (self.cache_size * 8) == 0:
            print("ASig0430 ts {} used size {} hf_dict {}, sigmoid {}, opt_pos {}, failed count {}, {}, {}, True {}, False {}".format(
