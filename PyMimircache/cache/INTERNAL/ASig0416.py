...
class ASig0416(Cache):
    ...
    def _update(self, req_item, **kwargs):
        ...
        if req_id in self.pin_dict:
            ...
            if 2 <= len(ts_list) < self.freq_boundary[0] + 2:
                # self.temp_dict["record"] = self.temp_dict.get("record", 0) + 1
                cur_dist = ts_list[-1] - ts_list[-2]
                self.low_freq_ts_count[len(ts_list) - 1 - 1][cur_dist] += 1
            # elif len(ts_list) < 2:
            #     self.temp_dict["tooShort"] += self.temp_dict.get("tooShort", 0) + 1
            # elif len(ts_list) >= self.freq_boundary[0]+2:
            #     self.temp_dict["tooLong"] = self.temp_dict.get("tooLong", 0) + 1
    ...
    def evict(self, **kwargs):
        ...
        # if LHD_chosen_key is None and low_freq_chosen_key is not None:
        #     self.evict_reasons["low_freq"] += 1
        #     del self.cache_dict[low_freq_chosen_key]
        #     return low_freq_chosen_key
        # if low_freq_chosen_key is None and LHD_chosen_key is not None:
        #     self.evict_reasons["Sigmoid"] += 1
        #     del self.cache_dict[LHD_chosen_key]
        #     return LHD_chosen_key
        # if LHD_chosen_key is not None and low_freq_chosen_key is not None:
        #     ### TEMP
        #     if self.ts - low_freq_access_time > 2 * self.cache_size:
        #         del self.cache_dict[low_freq_chosen_key]
        #         self.evict_reasons["low_freq"] += 1
        #         return low_freq_chosen_key
        #     else:
        ...
        #         del self.cache_dict[LHD_chosen_key]
        #         return LHD_chosen_key
        # if LHD_chosen_key is None and low_freq_chosen_key is None:
        #     raise RuntimeError("both key None")
