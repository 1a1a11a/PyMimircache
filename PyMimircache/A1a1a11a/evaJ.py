...
class EVA(Cache):
    class Class:
        ...
        def update(self):
            ...
            for i in range(self.max_age):
                ...
                self.events[i] = self.ewma_hits[i] + self.ewma_evicts[i]
        ...
        def reconfigure(self, per_access_cost):
            ...
            for i in range(self.max_age - 1, -1, -1):
                ...
                if events_up_to_now >= 1e-2:
                    ...
                    # changed
                    # self.eva[i] = (self.hit_probability[i] - per_access_cost * self.expected_lifetime[i] / events_up_to_now)
        ...
        def plot_eva(self, dat_name, cache_size=None):
            if PLOT_EVA and self.eva_plot_count >= 0:
                figname = "1127EVA_Value/{}/eva_{}_{}_{}.png".format(dat_name, self.name, cache_size,
                                                                     self.eva_plot_count)
    ...
    def __init__(self, cache_size=1000, update_interval=1280, max_age=20000, ewma_decay=0.8,
        ...
        print(
            "cache size {}, update interval {}, age_scaling {}, classification {}, decay coefficient {}, max_age {}".format(
        ...
        self.age_to_elements_deque = deque()

        self.current_ts = 0
        # this is the oldest ts of elements in the frist element_list in age_to_elements_deque
    ...
    def _verify(self, msg=""):
        assert len(self.age_to_elements_deque) <= self.max_age
        size = sum([len(i) for i in self.age_to_elements_deque])
        if msg == "d" or msg == "e" or msg == "f" or \
            ...
            assert size == len(self.access_time_d), \
                "msg {}, size different {} {}, age list size {}".format(
                    msg, size, len(self.access_time_d), len(self.age_to_elements_deque)
                )
        ...
        for n, element_list in enumerate(self.age_to_elements_deque):
            if element_list:
    ...
    def _verify2(self, msg=""):
        ...
        for e, t in self.access_time_d.items():
            ...
            age = self._get_age(e)
            # print("age {}/{} {}/{}/{} {}: {}".format(age, len(self.age_to_elements_deque),
            #                                          self.current_element_list_oldest_ts,
            #                                       current_ts, t, e, self.age_to_elements_deque))
            assert e in self.age_to_elements_deque[age], "{}: {} not in, age {} ({}/{}) {}". \
                format(msg, e, age, current_ts, t, self.age_to_elements_deque)
    ...
    def _get_age(self, element):
        ...
        # if left_over_age >= len(self.age_to_elements_deque[0]):
        #     scaled_age += 1
        return scaled_age
    ...
    def _update_age(self, element):
        ...
        # from the time order of age_to_elements_deque (old->new as right->left)
        ...
        if (self.current_ts - self.current_element_list_oldest_ts) % self.age_scaling == 0:
            self.age_to_elements_deque.appendleft(deque([element, ]))
            self.current_element_list_oldest_ts = self.current_ts
        else:
            self.age_to_elements_deque[0].append(element)

        if self.debugging_mode:
            assert len(self.age_to_elements_deque[0]) <= self.age_scaling, \
                "error first element size {} age scaling {}".format(len(self.age_to_elements_deque[0]),
                                                                    self.age_scaling)
        ...
        if len(self.age_to_elements_deque) > self.max_age:
            # print("wrap up")
            # element_max_age can be None, this is the situation when the element has been hit
            element_list_max_age = self.age_to_elements_deque.pop()
    ...
    def evict(self, **kwargs):
            ...
            if len(self.eva_sorted_age):
                ...
                if self.enable_classification:
                    for (class_name, age) in self.eva_sorted_age:
                        if age >= len(self.age_to_elements_deque):
                            continue
                ...
                # element_to_evict = self.age_to_elements_deque.pop()
                # while not element_to_evict:
                #     element_to_evict = self.age_to_elements_deque.pop()
                # evict_age = len(self.age_to_elements_deque)
                # evict_from = "no info"
                ...
                for e, t in self.access_time_d.items():
                    ...
                    # HIGH COMPLEXITY
                    element_list = self.age_to_elements_deque[evict_age]
        ...
        if element_to_evict is None:
            raise RuntimeError("cannot find element to evict {} {}".format(self.age_to_elements_deque,
                                                                           self.eva_sorted_age))
        # if socket.gethostname() != "node":
        ...
        #                 evict_from, len(self.age_to_elements_deque), len(self.top_eviction_deque)))
...
def run_data(dat, LRU_hr=None, cache_size=-1, update_interval=2000, max_age=20000, age_scaling=1,
             num_of_threads=os.cpu_count()):
    ...
    if LRU_hr is None:
        profiler_LRU = PyGeneralProfiler(reader, LRU, cache_size=CACHE_SIZE, bin_size=BIN_SIZE,
                                         num_of_threads=NUM_OF_THREADS)
        profiler_LRU.plotHRC(figname="1127vscsi/{}/LRU_{}.png".format(dat, dat), no_clear=True, no_save=False,
                             label="LRU")
    else:
                    ...
                    for age_scaling in [1, 2, 5, 10, 20, 100, 200]:
                        for dat in ["w92", "w106", "w78"]:
                            futures_to_params[ppe.submit(run_data, dat, LRU_HR_dict[dat], CACHE_SIZE,
                                                         ui, ma, age_scaling, os.cpu_count() // max_workers)] = (
                            ma, ui, age_scaling, dat)
            ...
            for age_scaling in [1, 2, 5, 10, 20, 100]:
                for cls in [True, False]:
                    ...
                    if os.path.exists(figname):
                        ...
                        continue
                    p0.plotHRC(no_clear=True, no_save=True, label="LRU")
                    p = PyGeneralProfiler(reader, EVA, cache_size=CACHE_SIZE, bin_size=BIN_SIZE,
                                          num_of_threads=NUM_OF_THREADS,
