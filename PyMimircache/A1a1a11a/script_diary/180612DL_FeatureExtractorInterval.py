...
class FeatureExtractorReq:
    ...
    def initialize(self, dat, dat_type):
        ...
        self.dat_name = os.path.basename(dat)
    ...
    def extract(self, feature_types=("label", "freq", "left_neighbour_freq",
        ...
        np.savetxt("{}/{}.index".format(OUTPUT_FOLDER, self.dat_name), self.kept_rec_ind)
...
def run_cphy_parallel(max_workers=os.cpu_count()):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as ppe:
        ...
        for futures in as_completed(futures_dict):
            ...
            print("w{} finished {}/96".format(futures_dict[futures], n_finished))


def run_akamai():
    from concurrent.futures import ProcessPoolExecutor, as_completed
    INPUT_FOLDER = "/home/jason/ALL_DATA/akamai3/layer/1/"
