...
def read_data(dat, feature_type, y_type):
    print("read {} {} {}".format(dat, feature_type, y_type))
...
def split_data(X, Y, split_ratio=(6,2,2)):
...
def gen_data_binary(X, Y, batch_size, step=1):
...
def gen_data_rand_binary(X, Y, batch_size, diff_thresh=200, n_batch=2000000):
    n_samples = X.shape[0]
    n_features = X.shape[1] * 2
    batch_ind = 0
    while batch_ind < n_batch:
        ...
        batch_Y1 = Y[rand1]
        batch_Y2 = Y[rand2]
        keep_pos = (np.abs(batch_Y1 - batch_Y2) > diff_thresh).reshape(-1)
        ...
        # use minus will reduce accuracy
...
def gen_data_rand_multiclass(X, Y, batch_size, diff_thresh=200, n_batch=2000000):
    n_samples = X.shape[0]
    n_features = X.shape[1] * 2
    batch_ind = 0
    while batch_ind < n_batch:
...
def gen_all_data_binary(X, Y, diff_thresh=200):
    n_samples = X.shape[0]
    n_features = X.shape[1] * 2
    ...
    for i in range(n_samples):
        if i % 200 == 0:
            ...
            t0 = time.time()
        keep_pos = (np.abs(Y - Y[i]) > diff_thresh).reshape(-1)
...
def gen_data_regression(X, Y, batch_size, diff_thresh=200):
    n_samples = X.shape[0]
    batch_ind = random.randint(0, n_samples % batch_size)
    while batch_ind + batch_size < n_samples:

        batch_Y = Y[batch_ind: batch_ind+batch_size] - diff_thresh
        batch_X = X[batch_ind: batch_ind+batch_size, :]

        batch_Y[batch_Y<=0] = 1
        batch_Y = np.log10(batch_Y)
...
def test():
    ...
    gen_all_data_binary(X_train, Y_train)
