...
program_data_t parda_omp_input(char inputFileName[], 
	program_data_t* pdt_a, long begin, long end, int pid, int psize);
void parda_omp_free(program_data_t* pdt_a, int psize);
void parda_omp_stackdist(char* inputFileName, long lines, int nthreads, long cache_size, float* return_array);
